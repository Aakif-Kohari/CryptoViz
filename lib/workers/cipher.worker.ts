...
async function getDispatcher(cipherId: string): Promise<CipherDispatcher> {
  switch (cipherId) {
    ...
    case "grain128": {
      const mod = await import("../cipher/symmetric/grain128");
      return { encrypt: mod.encrypt, decrypt: mod.decrypt };
    }
    case "3way": {
    ...
    case "goldwasser-micali": {
      const mod = await import("../cipher/asymmetric/goldwasser-micali");
      return { encrypt: mod.encrypt, decrypt: mod.decrypt };
    }
    case "ed25519": {
    ...
    case "md2": {
      const mod = await import("../cipher/hash/md2");
      return { encrypt: mod.encrypt, decrypt: mod.decrypt };
    }
    case "gost-r34-11-94": {
    ...
    case "snefru": {
      const mod = await import("../cipher/hash/snefru");
      return { encrypt: mod.encrypt, decrypt: mod.decrypt };
    }
    case "pbkdf2": {
      return {
