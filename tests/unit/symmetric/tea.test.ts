...
import { encrypt, decrypt, TEST_VECTORS } from '@/lib/cipher/symmetric/tea'
...
describe('TEA', () => {
  ...
  it('round-trips: decrypt(encrypt(x)) === x', () => {
    ...
    const plaintext = '48656c6c6f21212a'
  ...
  it('rejects a key that is not 128 bits', () => {
    expect(() => encrypt('0000000000000000', '00'.repeat(8))).toThrow(/128-bit key/)
  })

  it('rejects input that is not a multiple of 8 bytes', () => {
    expect(() => encrypt('001122', '000102030405060708090a0b0c0d0e0f')).toThrow(/multiple of 8 bytes/)
  })
})
